...
student = (name = "Zerol Acqua", kerberos_id = "zerolacqua")
...
Submission by: **_$(student.name)_** ($(student.kerberos_id)@outlook.com)
"""
...
function geometric(p::Real)
	if p==0
	throw(ArgumentError("p shouldn't be 0"))  
	end
...
it will always be 1, because it takes exactly one time to recover.
"""
...
my_mean=sum(large_experiment)/length(large_experiment)
...
let
	...
	# your code here
	vline!(ones(length(large_experiment))*my_mean,ls=:dash)
...
@bind p Slider( 0 : 0.1 : 1,show_value=true)

# ╔═╡ 562202be-5eac-46a4-9542-e6593bc39ff9
@bind N Slider( 0 : 1 : 100000,show_value=true)

# ╔═╡ e8d2a4ab-b710-4c16-ab71-b8c1e71fe442
function show_plot(p,N)
...
```math
\sum_{k=1}^{\infty} P_k = \sum_{k=1}^{\infty}p(1-p)^{k-1} \cdots = p\sum_{k=0}^{\infty}(1-p)^{k} = p\cdot \dfrac{1}{1-(1-p)} =1
...
function sum_geo_series()
    ...
    result=plot(T,get_Ps(p,N),label="Ps")
	bar(result,xs,ps,label="experiment")
end
sum_geo_series()
end
...
# ╔═╡ c0d6e575-743c-45a2-a75e-a0b77fc30bad
md"""
- abs(Ps-ps), sqrt.((Ps-ps).^2) and so on 

- the error depends on the experiment time. the more times we test, the less the error is.
"""
...
N=10
x=rand(N)
result = plot(x,ones(Int,N),label="rand",seriestype=:scatter,xlims=[0,1],ylims=[0,1])
bar!(result,cumulative,ones(Int,length(cumulative)),xlims=[0,1])
end
...
md"""
```math
C_n := \sum_{k=1}^n P_k = \sum_{k=1}^{n}p(1-p)^{k-1} = p\sum_{k=0}^{n-1}(1-p)^{k} = p\cdot \dfrac{1-(1-p)^n}{1-(1-p)} =1-(1-p)^n
```
"""
...
> Zerol Acqua: wtf?   $C_{n} \le r \le C_{n+1}$ makes sense.
...
md"""
```math
n(r,p) =  \lfloor \dfrac{\log(1-r)}{\log(1-p)} \rfloor 
```
"""
...
samples = geometric_fast.(ones(10_000)*10^-10)

# ╔═╡ c37bbb1f-8f5e-4097-9104-43ef65aa1cbd
histogram(samples, bins=50, title="histogram ", legend=:none)
...
let
atm_N=500
large_atm_experiment=[atmosphere(0.55,10,10^7) for i in range(1,atm_N)]
atm_ks,atm_vs=probability_distribution(large_atm_experiment)
bar(atm_vs,atm_ks,label=nothing)
end
...
# ╔═╡ a57791f3-40c6-4a74-aef2-3a2dcdfd503f
let 
atm_N=1000
large_atm_experiment=[atmosphere(0.55,10,10^7) for i in range(1,atm_N)]
atm_ks,atm_vs=probability_distribution(large_atm_experiment)
bar(atm_vs,atm_ks,label=nothing,yaxis=:log)
end
...
@bind atm_step Slider(1:100000)

# ╔═╡ d972be1f-a8ad-43ed-a90d-bca358d812c2
let 
atm_N=1000
large_atm_experiment=[atmosphere(0.55,10,atm_step) for i in range(1,atm_N)]
atm_ks,atm_vs=probability_distribution(large_atm_experiment)
bar(atm_vs,atm_ks,label=nothing)
end
