...
integers = [j for j=1:1000 if j*(1/j) ≠ 1]

# ╔═╡ 8efcaaeb-4900-404b-ae59-65db0bde8790
error=abs(integers[1]*(1/integers[1])-1)
...
ten_twelve = FirstRankOneMatrix([x for x in 1:10],[x for x in 1:12]) # Your code here
...
# wtf ????
# you need complete Base.size() and Base.getindex() functions below
# then this cell can display correctly
# that's really weird
RankOneMatrix([1,2,3,4])

# ╔═╡ d3e7c8d1-4b4a-47b6-9c96-150333078f42
RankOneMatrix([1,2,3,4],[1,2,3])
...
function matvec(M::RankOneMatrix, x)
	...
	return M.v*(M.w'*x) # Your code here
end
...
RankTwoMatrix(RankOneMatrix(1.0:10.0),RankOneMatrix(0.0:0.1:0.9))
...
function Base.getindex(M::LowRankMatrix, i, j)
	return sum(matrix[i,j] for matrix in M.Ms) # Your code here
end
...
function matvec(M::LowRankMatrix, x)
	...
	return sum(matvec(matrix,x) for matrix in M.Ms) # Your code here
end
...
From rank $\lceil\dfrac{n}{2}\rceil$ (exactly same when $n$ is an even, just exceeded when $n$ is an odd)

Because a rank-k matrix should use k rank-1 matrices which need $2kn$ entries in total. Solve the equation $2kn=n^2$ and get $k=\dfrac{n}{2}$
"""
...
answers=md"""
Only one singular value is approximately non-zero.

That makes sense because this matrix is rank-1
"""
...
function k_rank_ones(k, m, n)
	return collect(sum(RankOneMatrix(rand(m),rand(n)) for i=1:k))
end
...
numerical_rank(k_rank_ones(4,5,3))

# ╔═╡ 9813e9f9-9970-4a8b-b3ca-e6a699e6fda4
numerical_rank(k_rank_ones(4,2,3))

# ╔═╡ c8829a12-917a-4ff4-87c9-fe2b25aaa99c
md"""
From the results shown above
- When $m\geq k \land n\geq k$, the answer equals to $k$.
- When $m< k \lor n< k$, the answer equals to $\mathrm{min}(m,n)$.
"""
