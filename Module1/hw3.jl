...
begin
	function find_double(charcter)
		sample_freq_matrix[index_of_letter(charcter),index_of_letter(charcter)]>0.0
	end
	double_letters = filter(find_double,alphabet) # replace with your answer
...
function completion_cache(grams)
	...
	# your code here
	for word_pair in grams
		next_word=pop!(word_pair)

		if haskey(cache, word_pair)
			if !(next_word in cache[word_pair])
				push!(cache[word_pair],next_word)
			end
		else
			cache[word_pair]=[next_word]
		end
	end
	cache
end
