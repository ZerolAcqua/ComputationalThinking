...
function greedy_seam(energies, starting_pixel::Int)
	...
	for x in 1:m-1
		last_col=result[x]
		result[x+1] = argmin(arg->energies[x+1,Int(arg)], max(1,last_col-1):min(n,last_col+1))
	end
	result
...
function least_energy(energies, i, j)
	...
	## induction
	next_jump = j
	next_least_energy = 100000.0
	for k in max(1,j-1):min(n,j+1)
		next_energy, temp = least_energy(energies,i+1,k)
		if next_energy < next_least_energy
			next_least_energy=next_energy
			next_jump=k
		end
	end

	return (energies[i,j]+ next_least_energy,next_jump)
end
...
function recursive_seam(energies, starting_pixel)
	...
	result[1] = starting_pixel
	for i in 2:m
		tmp,result[i] = least_energy(energies, i-1,result[i-1] )
	end
	result
...
- When considering one recursion in the recursive_seam function, it explores all possible choices, either three or two choices. Even in the inner recursion, it continues to explore all possible choices. In order to complete the overall recursion, this algorithm needs to search through all possible seams, making it an exhaustive search.
- Due to the fact that it examines all possible routes without considering boundary situations, we need to check approximately $3^{m-1}$ seams. However, the value of $n$ does not affect the total number of seams, as the seam is constrained by continuity.
"""
...
function memoized_least_energy(energies, i, j, memory::Dict)
	...
	for k in max(1,j-1):min(n,j+1)
		if haskey(memory,(i+1,k))
			next_energy, temp = memory[(i+1,k)]
		else
			next_energy, temp = memoized_least_energy(energies,i+1,k,memory)
		end
		if next_energy < next_least_energy
			next_least_energy=next_energy
			next_jump=k
	...
	memory[(i,j)] = (energies[i,j] + next_least_energy, next_jump)
	return memory[(i,j)]
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	result[1] = starting_pixel
	for i in 2:m
		tmp,result[i] = memoized_least_energy(energies, i-1,result[i-1], memory)
	end
	result
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix)
	...
	next_least_energy = 100000.0
	for k in max(1,j-1):min(n,j+1)
		if memory[i+1,k]!=nothing
			next_energy, temp = memory[i+1,k]
		else
			next_energy, temp = matrix_memoized_least_energy(energies,i+1,k,memory)
		end
		if next_energy < next_least_energy
			next_least_energy=next_energy
			next_jump=k
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	result[1] = starting_pixel
	for i in 2:m
		tmp,result[i] = matrix_memoized_least_energy(energies, i-1,result[i-1], memory)
	end
	result
...
function least_energy_matrix(energies)
	...
	for i in m-1:-1:1
		...
		for j in 1:n
			result[i,j]=result[i,j] + findmin(result[i+1,y] for y in max(1,j-1):min(n,j+1))[1]
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	for i in 2:m
		j = result[i-1]
		result[i]=argmin(arg->least_energies[i,arg],max(1,j-1):min(n,j+1))
	end
	result
end
